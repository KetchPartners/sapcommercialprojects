...
Bring this website to your local environment by cloning the git repo with:
```
...
The local repo is a copied version of the remote repositories code base at a specific point in time.  The remote respositoiry is often called ```origin```.
...
When a developer brings it local, they often create a branch to represent the specific change the developer is going to make.  In agile methodologies, it is often tied to a board card (issue or new enhacement).  To create a local branch:
```
git branch issue-12
```
...
Please complate the training exercises outlined [next page](/sapcommercialprojects/training-git-exercise.html)
